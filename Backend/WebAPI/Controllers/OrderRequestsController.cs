...
// Handling order request workflows
[Authorize]
[ApiController]
...
public class OrderRequestsController : ControllerBase
{
    ...
    // Creating a customer order request
    [HttpPost]
    ...
    // Getting the current customer's orders
    [HttpGet("my-orders")]
    [Authorize(Roles = UserRoles.Customer)]
    ...
    // Getting all order requests for staff and admins
    [HttpGet("all")]
    ...
    // Getting pending order requests
    [HttpGet("pending")]
    [Authorize(Roles = UserRoles.Admin + "," + UserRoles.Staff)]
    ...
    // Cancelling a pending order request
    [HttpPatch("{id}/cancel")]
    ...
    // Creating an invoice from an order request
    ...
    // Marking an order request as completed
