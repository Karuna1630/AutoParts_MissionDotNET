    ...
    <div className="h-[calc(100vh-160px)] flex gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex-[2] flex flex-col gap-6 h-full overflow-hidden">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {selectedCustomer && (
              <button onClick={() => {setSelectedCustomer(null); setCustomers([])}} className="text-xs text-red-500 font-bold hover:underline">Change</button>
            )}
          </div>
          ...
          {!selectedCustomer ? (
            <div className="relative">
              ...
              />
              {customers.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl z-10 overflow-hidden">
                  {customers.map(c => (
                    <button 
                      key={c.id} 
                      onClick={() => {setSelectedCustomer(c); setCustomers([]); setCustomerSearch('')}}
                      className="w-full p-4 text-left hover:bg-slate-50 flex items-center gap-4 transition"
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider">{c.phone} | {c.email}</p>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="flex items-center gap-6 animate-in slide-in-from-left duration-300">
              <div className="flex items-center gap-4 flex-1">
                <div className="w-12 h-12 bg-blue-600 text-white rounded-2xl flex items-center justify-center font-bold shadow-lg shadow-blue-600/20">
                  {selectedCustomer.fullName[0]}
                </div>
                <div>
                  <p className="text-sm font-bold text-slate-800">{selectedCustomer.fullName}</p>
              ...
              <div className="flex-1">
                <select 
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20"
                >
      ...
      <div className="w-96 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col overflow-hidden h-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.length > 0 ? (
            cart.map(item => (
              <div key={item.id} className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100 group animate-in slide-in-from-right duration-300">
                <div className="flex justify-between items-start gap-3">
                  <div className="flex gap-3 flex-1 min-w-0">
                    <div className="w-12 h-12 bg-white rounded-xl border border-slate-100 flex-shrink-0 overflow-hidden flex items-center justify-center">
                      {item.imageUrl ? (
                        <img src={item.imageUrl} alt={item.name} className="w-full h-full object-cover" />
                      ) : (
                        <FiShoppingCart size={16} className="text-slate-200" />
                      )}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-slate-800 leading-tight truncate">{item.name}</p>
                      <p className="text-[10px] font-medium text-slate-500 mt-1 uppercase">Rs.{item.price.toFixed(2)} / unit</p>
                    </div>
                  </div>
                ...
                <div className="flex justify-between items-center mt-3">
                  <div className="flex items-center gap-3 bg-white border border-slate-100 rounded-xl px-2 py-1 shadow-sm">
                    <button onClick={() => updateQuantity(item.id, -1)} className="text-slate-400 hover:text-blue-600 transition-colors"><FiMinus size={14} /></button>
                    <span className="text-xs font-black text-slate-800 min-w-[20px] text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, 1)} className="text-slate-400 hover:text-blue-600 transition-colors"><FiPlus size={14} /></button>
                  </div>
                  <span className="text-sm font-black text-slate-900">Rs.{(item.price * item.quantity).toFixed(2)}</span>
                </div>
              </div>
            ))
          ) : (
