...
import { FaFileInvoiceDollar, FaShieldAlt, FaSignOutAlt } from 'react-icons/fa';

const sidebarGroups = [
  {
    title: 'SALES',
    items: [
      { id: 'sales', label: 'Sales Invoices', icon: FaFileInvoiceDollar, path: '/staff/sales' },
    ],
  },
];
...
const StaffLayout = () => {
  ...
  const user = JSON.parse(localStorage.getItem('authUser') || '{"fullName": "Staff", "role": "Staff"}');
      ...
      <aside className="fixed left-0 top-0 z-40 h-screen w-72 bg-[#0a0a0f] text-slate-400 shadow-2xl">
        <div className="flex h-24 items-center gap-4 px-8">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] font-black uppercase tracking-[0.2em] text-violet-400 opacity-80">Staff Panel</p>
          </div>
        </div>
