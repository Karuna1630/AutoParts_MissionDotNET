...
const AdminProfile = () => {
  ...
  const handleUpdate = async (values, { setSubmitting, setStatus }) => {
    try {
      ...
      if (response.success) {
        ...
        // Notify other components (like AdminLayout header)
  ...
  if (loading) return <div className="flex justify-center p-20"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-slate-800"></div></div>;
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Manage your administrator account and preferences.</p>
        </div>
        {!isEditing && (
          <button 
            onClick={() => setIsEditing(true)}
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-6 py-3 rounded-xl font-bold shadow-lg transition active:scale-95"
          >
          ...
          <div className="pt-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 text-xs font-bold border border-emerald-100">
                    <FiCheckCircle className="text-[10px]" /> System Verified
                  </span>
                  <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-bold border border-slate-200 uppercase">
                    {profile.role || 'Admin'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <StatBox icon={FiUsers} label="STAFF" value="12" />
                <StatBox icon={FiFileText} label="LOGS" value="84" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-50 flex items-center gap-3 text-slate-800">
                ...
                {isEditing && (
                  <div className="md:col-span-2 flex justify-end gap-4 mt-8 pt-6 border-t border-slate-50">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-8 py-3 rounded-xl font-bold shadow-lg transition disabled:opacity-50"
                    >
        ...
        <Field
          ...
          className={`w-full border rounded-xl px-4 py-3 text-sm font-semibold transition focus:outline-none focus:ring-4 ${
            disabled 
              ...
              : 'bg-slate-50 border-slate-200 text-slate-800 focus:ring-slate-100 focus:border-slate-500'
          }`}
        />
