...
const AdminLayout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 z-40 h-screen w-64 border-r border-slate-200 bg-[#0B0F1A] text-slate-400">
        <div className="flex h-20 items-center gap-3 px-6">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-indigo-600 shadow-lg shadow-violet-500/40">
          ...
          <div>
            ...
            <p className="text-[9px] uppercase tracking-[0.2em] text-violet-400">Admin Panel</p>
          </div>
        </div>
      ...
      <div className="flex-1 pl-64">
        {/* Top Header */}
        <header className="sticky top-0 z-30 flex h-20 items-center justify-end bg-white/80 px-8 backdrop-blur-md border-b border-slate-100">
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <div className="text-right">
                <p className="text-sm font-black uppercase tracking-tight text-slate-800 group-hover:text-violet-600">{user.fullName}</p>
                <span className="rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-bold text-violet-600">
                  {user.role || 'Admin'}
                </span>
              </div>
              <div className="flex h-10 w-10 overflow-hidden items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 text-white shadow-sm transition-all group-hover:shadow-md border-2 border-transparent group-hover:border-violet-200">
                {user.avatarUrl ? (
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-3 w-48 origin-top-right rounded-2xl border border-slate-100 bg-white p-2 shadow-2xl shadow-slate-200/50 animate-in fade-in zoom-in-95 duration-200">
                <Link 
                  ...
                  className="flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-violet-600"
                >
